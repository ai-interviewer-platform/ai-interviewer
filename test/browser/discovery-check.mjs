...
try {
  ...
  console.log('Passed: core surfaces, contrast scan, and home reflow');
  ...
  assert.equal(await page.evaluate(() => JSON.parse(sessionStorage.getItem('interview-prototype')).reviewOpened), true);
  await page.reload();
  assert.equal(await page.evaluate(() => JSON.parse(sessionStorage.getItem('interview-prototype')).reviewOpened), true);
  ...
  console.log('Passed: recorded review progress persistence and empty state');
  ...
  for (const topic of topics) {
    for (const leaf of topicTrees[topic.id].flatMap((branch) => branch.leaves)) {
      ...
      assert.equal(await page.locator('.practice-problems > li').count(), leaf.exercises.length);
