import { directions, exercises, scenes, parseRoute, fixtureResult, initialAttempt, updateAttempt } from './model.js';

import { directionPicker, homeScreen, roadmapScreen, getPracticeLeaf, problemDrawer } from './discovery.js';
...
const link = (label, path, style = '', glyph = '') => `<a class="button ${style}" href="#${path}">${glyph ? icon(glyph) : ''}<span>${label}</span></a>`;
...
const state = {
  ...
  layout: 40, codeSize: 14, reviewState: '', sampleRoute: 'sample', replaying: false, replayPosition: 78, direction: 'fieldnotes', reviewOpened: false,
};
try {
  ...
  if (saved) {
    ...
    if (directions.some((item) => item.id === saved.direction)) state.direction = saved.direction;
...
function persist() {
  try {
    sessionStorage.setItem('interview-prototype', JSON.stringify({ personal: state.personal, sample: state.sample, retry: state.retry, sampleRoute: state.sampleRoute, historyDeleted: state.historyDeleted, direction: state.direction, reviewOpened: state.reviewOpened }));
    return true;
  } catch { return false; }
}
...
function chrome(content, workspace = false) {
  return `<div class="prototype-bar"><span>Design prototype <span class="bar-detail">/ Fictional sessions. Nothing is recorded or sent.</span></span>${button('Explore screens', 'scenes', 'quiet small', 'expand')}</div>
  <header class="app-header"><a class="wordmark" href="#welcome"><span class="brand-mark" aria-hidden="true">[·]</span>Interview trainer</a>
  ...
  <div class="header-end">${link('Preferences', 'preferences', 'quiet', 'settings')}<span class="avatar" aria-label="Demo profile">D</span></div></header>
  ${workspace ? '' : `<div class="design-toolbar"><span>Design study <strong>${directions.find((item) => item.id === state.direction).name}</strong></span>${directionPicker(state.direction)}</div>`}<main id="main" tabindex="-1" class="${workspace ? 'workspace-main' : 'page-main'}">${content}</main>
  ${workspace ? '' : '<footer class="page-footer"><span>Practice, with perspective.</span><a href="#system">Design system & motion</a><span>Prototype · v0.1</span></footer>'}`;
}
...
function render(navigation = false) {
  ...
  const requestedDirection = route.params.get('direction');
  if (directions.some((item) => item.id === requestedDirection)) state.direction = requestedDirection;
  document.documentElement.dataset.direction = state.direction;
  if (route.page === 'review' && !isSample()) { state.reviewOpened = true; persist(); }
  const screens = { welcome, roadmap, sessions, setup, sample: workspace, interview: workspace, review: workspace, retry: workspace, complete, related, preferences, system };
...
document.addEventListener('change', (event) => {
  ...
  if (input.id === 'direction') { state.direction = input.value; persist(); const url = new URL(location.href); url.hash = url.hash.replace(/([?&])direction=[^&]*&?/, '$1').replace(/[?&]$/, ''); history.replaceState(null, '', url); render(); notify('Visual direction changed. Your practice state is unchanged.'); }
